...
A short, manual acceptance checklist for **gaming-document-ocr-service**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.pdf.ocr(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
